...
class Ensemble(object):
    ...
    def cross_pair(self):
        ...
        for weight_name in self.cnn_features_dir_name:
            ...
            X = pickle_load_data(os.path.join(feature_dir, "X.pkl"))
            weight, b = pickle_load_data(os.path.join(feature_dir, "weights.pkl"))
            print(X.shape)
            # clf = pickle_load_data(os.path.join(feature_dir, model_name + "_model.pkl"))
            y = self.data.y
            train_X = X[np.array(self.train_idx), :]
...
if __name__ == '__main__':
    ...
    # d.single_ensemble_result_logistic([1e-3, 1e0])
    # d.single_ensemble_result_logistic([1e-5, 1e0, 1e5])
    # d.single_ensemble_result_logistic([1e-5, 1e-3, 1e0, 1e5])
    # d.single_ensemble_result_logistic([1e-5, 1e-3, 1e0, 1e3, 1e5])
