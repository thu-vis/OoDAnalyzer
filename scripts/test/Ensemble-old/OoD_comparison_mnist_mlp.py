...
def TPR95(x, y):
    ...
    total = 0.0
    flag = 1
    for idx, delta in enumerate(np.arange(x.min(), x.max(), gap)):
...
class Comparison(object):
    def __init__(self, dataname):
        ...
        self.test_idx = np.array(range(50000, 78726))
        self.test_redundant_idx = np.array(range(60000, 78726))
