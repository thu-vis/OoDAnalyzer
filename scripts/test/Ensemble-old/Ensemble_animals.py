...
# from scripts.test.Ensemble.OoD_comparison import Comparison
...
class Ensemble(object):
    ...
    def uncertainty(self, model_list=None, step=None):
        train_predys = []
        test_predys = []
        for model_name in model_list:
            for weight_name in self.cnn_features_dir_name:
                ...
                feature_dir = os.path.join(config.data_root, self.dataname,
                                           "feature", weight_name)
                model_dir = os.path.join(config.data_root, self.dataname,
                                         "feature", weight_name, step)
                X = pickle_load_data(os.path.join(feature_dir, "X.pkl"))
                clf = pickle_load_data(os.path.join(model_dir, model_name + "_model.pkl"))
                y = self.data.y
        ...
        pickle_save_data(os.path.join(config.data_root,
                                        self.dataname,
                                        "all_entropy." + step + "pkl"), all_entropy)
        return

    def train_ensemble_models(self, model_name="svm", c = 1, step=None):
        for weight_name in self.cnn_features_dir_name:
            logger.info("processing {}".format(weight_name))
            feature_dir = os.path.join(config.data_root, self.dataname,
                                       "feature", weight_name)
            ...
            test_y = y[np.array(self.test_idx)]
            if model_name == "svm":
                clf = SVC(kernel="linear", C=c, verbose=1)
            elif model_name == "rf":
                clf = RandomForestClassifier(n_estimators=500, max_depth=100)
            elif model_name == "xgboost":
                clf = XGBClassifier(n_estimators=50, max_depth=5, colsample_bytree=0.8)
            elif model_name == "logistic":
                clf = LogisticRegression()

            clf.fit(train_X, train_y)
            print(weight_name, clf.score(train_X, train_y), clf.score(test_X, test_y))
            pickle_save_data(os.path.join(model_dir, model_name + "_model.pkl"), clf)



    def get_entropy(self, model_list, suffix):
        if not os.path.exists(os.path.join(config.data_root,
                                                    self.dataname,
                                                    "all_entropy_diff" +suffix+".pkl")):
            self.uncertainty(model_list=model_list, step=suffix)
...
if __name__ == '__main__':
    ...
    #     e.train_ensemble_models(model_name, step="step1")
    e.get_entropy(["svm","logistic", "xgboost", "rf"],"step1")
#     # e.train_ensemble_models("logistic")
#     # for c in [2^1, 2^3, 2^5, 2^7, 2^9, 2^11]:
#     for c in [1]:
#         e = Ensemble(config.dog_cat)
#         # for model_name in ["svm","xgboost", "rf"]:
