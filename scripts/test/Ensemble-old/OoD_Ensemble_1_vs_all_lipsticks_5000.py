...
class Ensemble(object):
    ...
    def __init__(self, dataname):
        ...
        original_train_idx = self.data.train_idx
        self.valid_idx = self.data.valid_idx
        original_test_idx = self.data.test_idx
        original_test_redundant_idx = self.data.test_redundant_idx

        idx = np.array(range(len(original_train_idx)))
        np.random.seed(123)
        np.random.shuffle(idx)
        self.train_idx = np.array(original_train_idx)[idx[:5000]]

        original_normal_test = [i for i in original_test_idx if i not in original_test_redundant_idx]
        idx = np.array(range(len(original_normal_test)))
        np.random.seed(123)
        np.random.shuffle(idx)
        self.test_idx = np.array(original_normal_test)[idx[:2000]].tolist()\
                        + original_test_redundant_idx
        self.test_redundant_idx = original_test_redundant_idx
    ...
    def grid_search_logistic(self):
        for weight_name in self.cnn_features_dir_name:
            ...
            grid_search_dir = os.path.join(feature_dir, "logistic_5000")
            ...
            for C in [2e-7, 2e-5, 2e-3, 2e-1, 2e1, 2e3, 2e5, 2e7]:
                for kernel in ["linear", "rbf"]:
                    if kernel == "linear":
                        ...
                        clf.fit(train_X, train_y)
                        model_name = "kernel_{}_C_{}".format(kernel, C)
                        ...
                        for gamma in [2e-5, 2e-3, 2e-1, 2e1, 2e3]:
                            ...
                            clf.fit(train_X, train_y)
                            model_name = "kernel_{}_C_{}_gamma{}".format(kernel, C, gamma)
                            model_path = os.path.join(grid_search_dir, model_name)
                            pickle_save_data(model_path, clf)
